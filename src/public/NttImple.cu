...
__device__ __inline__ void butt_ntt_local_mont(uint64_t &a, uint64_t &b,
                                               const uint64_t &wR,
                                               const uint64_t p,
                                               const uint64_t nprime) {
  const uint64_t two_p = 2 * p;
  uint64_t U = mont_mul_montified_const(b, wR, p, nprime);
  if (a >= two_p) a -= two_p;
  b = a + (two_p - U);
  a += U;
}
...
__device__ __inline__ void butt_intt_local_mont(word64 &x, word64 &y,
                                                const word64 &wR,
                                                const word64 &p,
                                                const word64 &nprime) {
  const word64 two_p = 2 * p;
  ...
  if (T & 1) new_x += p;
  x = (new_x >> 1);
  y = mont_mul_montified_const(T, wR, p, nprime);
}
...
__global__ void Ntt8PointPerThreadPhase1Mont(
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (N / 8 * num_prime);
    ...
    const uint64_t prime = primes[np_idx];
    const uint64_t nprime = nprimes[np_idx];
    int N_init = 2 * m_idx * t + t / 4 / radix * WarpID + Warp_t +
                 pad * (t_idx / (radix * pad));
    for (int j = 0; j < 8; j++) local[j] = *(a_np + N_init + t / 4 * j);
    __syncthreads();
    int eradix = 8 * radix;
    int tw_idx = m + m_idx;
    for (int j = 0; j < 4; j++) {
      butt_ntt_local_mont(local[j], local[j + 4], W_mont[np_idx * N + tw_idx],
                          prime, nprime);
    }
    for (int j = 0; j < 2; j++) {
      butt_ntt_local_mont(local[4 * j], local[4 * j + 2],
                          W_mont[np_idx * N + 2 * tw_idx + j], prime, nprime);
      butt_ntt_local_mont(local[4 * j + 1], local[4 * j + 3],
                          W_mont[np_idx * N + 2 * tw_idx + j], prime, nprime);
    }
    for (int j = 0; j < 4; j++) {
      butt_ntt_local_mont(local[2 * j], local[2 * j + 1],
                          W_mont[np_idx * N + 4 * tw_idx + j], prime, nprime);
    }
    for (int j = 0; j < 8; j++) {
...
#pragma unroll
    for (int j = 8, k = radix / 2; j < radix + 1; j *= 8, k >>= 3) {
      ...
      for (int j2 = 0; j2 < 4; j2++) {
        butt_ntt_local_mont(local[j2], local[j2 + 4],
                            W_mont[np_idx * N + tw_idx2], prime, nprime);
      }
      for (int j2 = 0; j2 < 2; j2++) {
        butt_ntt_local_mont(local[4 * j2], local[4 * j2 + 2],
                            W_mont[np_idx * N + 2 * tw_idx2 + j2], prime,
                            nprime);
        butt_ntt_local_mont(local[4 * j2 + 1], local[4 * j2 + 3],
                            W_mont[np_idx * N + 2 * tw_idx2 + j2], prime,
                            nprime);
      }
      for (int j2 = 0; j2 < 4; j2++) {
        butt_ntt_local_mont(local[2 * j2], local[2 * j2 + 1],
                            W_mont[np_idx * N + 4 * tw_idx2 + j2], prime,
                            nprime);
      }
    ...
    if (tail == 1) {
      ...
      butt_ntt_local_mont(local[0], local[1], W_mont[np_idx * N + tw_idx2],
                          prime, nprime);
      butt_ntt_local_mont(local[2], local[3],
                          W_mont[np_idx * N + tw_idx2 + 1], prime, nprime);
      butt_ntt_local_mont(local[4], local[5],
                          W_mont[np_idx * N + tw_idx2 + 2], prime, nprime);
      butt_ntt_local_mont(local[6], local[7],
                          W_mont[np_idx * N + tw_idx2 + 3], prime, nprime);
      for (int l = 0; l < 8; l++) {
      ...
      butt_ntt_local_mont(local[0], local[2], W_mont[np_idx * N + tw_idx2],
                          prime, nprime);
      butt_ntt_local_mont(local[1], local[3], W_mont[np_idx * N + tw_idx2],
                          prime, nprime);
      butt_ntt_local_mont(local[4], local[6],
                          W_mont[np_idx * N + tw_idx2 + 1], prime, nprime);
      butt_ntt_local_mont(local[5], local[7],
                          W_mont[np_idx * N + tw_idx2 + 1], prime, nprime);
      butt_ntt_local_mont(local[0], local[1],
                          W_mont[np_idx * N + 2 * tw_idx2], prime, nprime);
      butt_ntt_local_mont(local[2], local[3],
                          W_mont[np_idx * N + 2 * tw_idx2 + 1], prime,
                          nprime);
      butt_ntt_local_mont(local[4], local[5],
                          W_mont[np_idx * N + 2 * tw_idx2 + 2], prime,
                          nprime);
      butt_ntt_local_mont(local[6], local[7],
                          W_mont[np_idx * N + 2 * tw_idx2 + 3], prime,
                          nprime);
      for (int l = 0; l < 8; l++) {
        temp[(eradix + pad) * Warp_t + 8 * WarpID + l] = local[l];
      }
...
__global__ void Ntt8PointPerThreadPhase2Mont(
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (N / 8 * num_prime);
    ...
    int tw_idx = m + m_idx;
    for (int j = 0; j < 4; j++) {
      butt_ntt_local_mont(local[j], local[j + 4],
                          W_mont[np_idx * N + tw_idx], prime, nprime);
    }
    for (int j = 0; j < 2; j++) {
      butt_ntt_local_mont(local[4 * j], local[4 * j + 2],
                          W_mont[np_idx * N + 2 * tw_idx + j], prime, nprime);
...
__global__ void Intt8PointPerThreadPhase2Mont(
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (N / 8 * num_prime);
    ...
    __syncthreads();
    for (int j = 0; j < 8; j++) {
      temp[set * 8 * radix + t_idx + t / 4 * j] = *(in_addr + N_init + t / 4 * j);
    }
    __syncthreads();
    for (int l = 0; l < 8; l++) {
      local[l] = temp[set * 8 * radix + 8 * t_idx + l];
    ...
    int tw_idx = m + m_idx;
    int tw_idx2 = (t / 4) * tw_idx + t_idx;
    const uint64_t *WInv = WInv_mont + N * np_idx;
    for (int j = 0; j < 4; j++) {
      butt_intt_local_mont(local[2 * j], local[2 * j + 1], WInv[4 * tw_idx2 + j], prime, nprime);
    }
    for (int j = 0; j < 2; j++) {
      butt_intt_local_mont(local[4 * j], local[4 * j + 2], WInv[2 * tw_idx2 + j], prime, nprime);
      butt_intt_local_mont(local[4 * j + 1], local[4 * j + 3], WInv[2 * tw_idx2 + j], prime, nprime);
    }
    for (int j = 0; j < 4; j++) {
      butt_intt_local_mont(local[j], local[j + 4], WInv[tw_idx2], prime, nprime);
    }
    for (int j = 0; j < 8; j++) {
      *(out_addr + N_init + t / 4 * j) = local[j];
    }
...
#pragma unroll
    for (int j = radix / 8, k = 32; j > 0; j >>= 3, k *= 8) {
      ...
      int tw_idx2b = j * tw_idx + m_idx2;
      for (int l = 0; l < 4; l++) {
        butt_intt_local_mont(local[2 * l], local[2 * l + 1], WInv[4 * tw_idx2b + l], prime, nprime);
      }
      for (int l = 0; l < 2; l++) {
        butt_intt_local_mont(local[4 * l], local[4 * l + 2], WInv[2 * tw_idx2b + l], prime, nprime);
        butt_intt_local_mont(local[4 * l + 1], local[4 * l + 3], WInv[2 * tw_idx2b + l], prime, nprime);
      }
      for (int l = 0; l < 4; l++) {
        butt_intt_local_mont(local[l], local[l + 4], WInv[tw_idx2b], prime, nprime);
      }
      for (int l = 0; l < 8; l++) {
        temp[(eradix + pad) * Warp_t + 2 * m_idx2 * k + t_idx2 + (k / 4) * l] =
            local[l];
    ...
    if (tail == 1) {
      butt_intt_local_mont(local[0], local[4], WInv[tw_idx], prime, nprime);
      butt_intt_local_mont(local[1], local[5], WInv[tw_idx], prime, nprime);
      butt_intt_local_mont(local[2], local[6], WInv[tw_idx], prime, nprime);
      butt_intt_local_mont(local[3], local[7], WInv[tw_idx], prime, nprime);
    } else if (tail == 2) {
      butt_intt_local_mont(local[0], local[2], WInv[2 * tw_idx], prime, nprime);
      butt_intt_local_mont(local[1], local[3], WInv[2 * tw_idx], prime, nprime);
      butt_intt_local_mont(local[4], local[6], WInv[2 * tw_idx + 1], prime, nprime);
      butt_intt_local_mont(local[5], local[7], WInv[2 * tw_idx + 1], prime, nprime);
      butt_intt_local_mont(local[0], local[4], WInv[tw_idx], prime, nprime);
      butt_intt_local_mont(local[1], local[5], WInv[tw_idx], prime, nprime);
      butt_intt_local_mont(local[2], local[6], WInv[tw_idx], prime, nprime);
      butt_intt_local_mont(local[3], local[7], WInv[tw_idx], prime, nprime);
